...
import { BookOpen, MessageSquareQuote, ScrollText, Moon, Sun, Download, Save, FolderOpen } from 'lucide-react';
...
const AyahBlock = Node.create({
  ...
  renderHTML({ HTMLAttributes }) {
    return [
      'div',
      mergeAttributes(HTMLAttributes, { 'data-type': 'ayah-block', class: 'ayah-block' }),
      ['p', 0],
      ['div', { class: 'ayah-ref' }, `Surah ${HTMLAttributes.surah || '—'} : Ayah ${HTMLAttributes.ayah || '—'}`],
    ];
  },
  addCommands() {
    return { setAyahBlock: () => ({ commands }) => commands.setNode(this.name) };
  },
  addKeyboardShortcuts() {
    return { 'Mod-1': () => this.editor.commands.setAyahBlock() };
  },
});
...
const HadithBlock = Node.create({
  ...
  addCommands() {
    return { setHadithBlock: () => ({ commands }) => commands.setNode(this.name) };
  },
  addKeyboardShortcuts() {
    return { 'Mod-2': () => this.editor.commands.setHadithBlock() };
  },
});
...
const CommentaryBlock = Node.create({
  ...
  defining: true,
  parseHTML() {
    return [{ tag: 'div[data-type="commentary-block"]' }];
  },
  renderHTML({ HTMLAttributes }) {
    return ['div', mergeAttributes(HTMLAttributes, { 'data-type': 'commentary-block', class: 'commentary-block' }), ['p', 0]];
  },
  addCommands() {
    return { setCommentaryBlock: () => ({ commands }) => commands.setNode(this.name) };
  },
  addKeyboardShortcuts() {
    return { 'Mod-5': () => this.editor.commands.setCommentaryBlock() };
  },
});

const STORAGE_KEY = 'muallim-editor-doc';
const MUALLIM_FILE_EXTENSION = '.muallim';
...
export function EditorPage() {
  ...
  const extensions = useMemo(
    () => [StarterKit.configure({ paragraph: false }), Placeholder.configure({ placeholder: 'Begin your ta’liq…' }), AyahBlock, HadithBlock, CommentaryBlock],
    [],
  );
  ...
  const editor = useEditor({
    extensions,
    content:
      '<div data-type="commentary-block"><p>بِسْمِ اللهِ الرَّحْمٰنِ الرَّحِيمِ</p></div><div data-type="commentary-block"><p>Write commentary, organize evidences, and preserve references with beauty.</p></div>',
  ...
  const exportPdf = async () => {
    ...
    if (!el) return;
    await html2pdf()
      .set({ margin: [20, 18], filename: 'muallim-notes.pdf', html2canvas: { scale: 2 }, jsPDF: { unit: 'mm', format: 'a4' } })
      .from(el)
      .save();
  };
  ...
  const saveLocalFile = () => {
    ...
    a.href = url;
    a.download = `muallim-notes${MUALLIM_FILE_EXTENSION}`;
    a.click();
  ...
  const importLocalFile = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const importedContent = parsed?.content;

      if (!importedContent) {
        throw new Error('Invalid file format.');
      }

      editor.commands.setContent(importedContent, false);
      localStorage.setItem(STORAGE_KEY, JSON.stringify(importedContent));
    } catch (error) {
      console.error('Failed to import file:', error);
      window.alert('Unable to import file. Please select a valid .muallim export.');
    } finally {
      event.target.value = '';
    }
  };
  ...
  return (
    <div className={dark ? 'dark' : ''}>
      <div className="min-h-screen bg-parchment text-stone-800 dark:bg-night dark:text-stone-200 transition-colors">
        <header className="sticky top-0 z-50 backdrop-blur border-b border-stone-300/50 dark:border-stone-700/40 bg-parchment/90 dark:bg-night/90">
          <div className="mx-auto max-w-6xl px-6 py-3 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => setDark((d) => !d)} className="toolbar-btn">{dark ? <Sun size={14} /> : <Moon size={14} />}</button>
              <input
                ref={fileInputRef}
                type="file"
                accept={`${MUALLIM_FILE_EXTENSION},application/json`}
...
function ToolbarButton({ icon, label, onClick }: { icon: JSX.Element; label: string; onClick: () => void }) {
  return (
    <button title={label} onClick={onClick} className="toolbar-btn">
      {icon}
